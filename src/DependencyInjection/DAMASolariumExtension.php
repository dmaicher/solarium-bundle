...
use DAMA\SolariumBundle\DataCollector;
...
class DAMASolariumExtension extends Extension
{
    public function load(array $configs, ContainerBuilder $container): void
    {
        ...
        // configure registry
        $registry = $container->register('solarium.client_registry', Psr11ClientRegistry::class);
        $registry->setPublic(true);
        $registry->setArguments([
            ServiceLocatorTagPass::register($container, $clients),
            array_keys($clients),
            array_key_exists($defaultClient, $clients) ? $defaultClient : null,
        ]);
    }
    ...
    private function registerDataCollector(ContainerBuilder $builder): void
    {
        $dataCollector = $builder->register('solarium.data_collector', DataCollector::class);
        $dataCollector->setPublic(true);
        $dataCollector->addMethodCall('setLogger', [
            new Reference('logger', ContainerInterface::IGNORE_ON_INVALID_REFERENCE),
        ]);
        $dataCollector->addMethodCall('setStopwatch', [
            new Reference('debug.stopwatch', ContainerInterface::IGNORE_ON_INVALID_REFERENCE),
        ]);
        $dataCollector->addTag('data_collector', [
            'template' => '@DAMASolarium/DataCollector/solarium.html.twig',
            'id' => 'solr',
        ]);
        $dataCollector->addTag('monolog.logger', ['channel' => 'solr']);
    }
}
